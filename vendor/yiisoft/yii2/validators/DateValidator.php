...
class DateValidator extends Validator
{
    ...
    public function init()
    {
        ...
        if ($this->min !== null && $this->tooSmall === null) {
            $this->tooSmall = Yii::t('yii', '{attribute} não pode ser menor que {min}.');
        }
        if ($this->max !== null && $this->tooBig === null) {
            $this->tooBig = Yii::t('yii', '{attribute} não pode ser maior que {max}.');
        }
        ...
        if ($this->max !== null && is_string($this->max)) {
            ...
            if ($timestamp === false) {
                throw new InvalidConfigException("Data máxima inválida: {$this->max}");
            }
            $this->max = $timestamp;
        }
        if ($this->min !== null && is_string($this->min)) {
            ...
            if ($timestamp === false) {
                throw new InvalidConfigException("Data mínima inválida: {$this->min}");
            }
            $this->min = $timestamp;
        }
    }
