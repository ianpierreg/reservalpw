...
class UniqueValidator extends Validator
{
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            $this->message = Yii::t('yii', '{attribute} "{value}" já foi utilizado.');
        }
        if ($this->comboNotUnique === null) {
            $this->comboNotUnique = Yii::t('yii', 'A combinação {values} de {attributes} já foi utilizada.');
        }
    }
    ...
    public function validateAttribute($model, $attribute)
    {
        ...
        foreach ($params as $value) {
            if (is_array($value)) {
                $this->addError($model, $attribute, Yii::t('yii', '{attribute} é inválido.'));
